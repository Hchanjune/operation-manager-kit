...
반환 타입은 추론 가능하므로 명시적 선언은 생략할 수 있습니다:
...
`ManagedContextWebFilter`는 `beforeCommit` 훅에서 최종 HTTP 응답 상태 코드로 `context.injectStatusCode(statusCode)`를 호출하여 `context.outcome`([OperationOutcome](API.ko.md#operationoutcome) 참고)을 도출합니다.

- `outcome == SERVER_ERROR`(5xx)이면 `onFailure`가 호출됩니다.
- 그 외의 모든 outcome(`UNAUTHENTICATED`(401), `FORBIDDEN`(403) 포함)은 `onSuccess`가 호출됩니다 — 요청이 정상적으로 처리되어 의도한 응답이 반환되었기 때문입니다 (예: Spring Security의 `AuthenticationEntryPoint`).
