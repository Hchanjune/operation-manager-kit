...
class ManagedEventHandlerAspectMetadataTest {
    ...
    fun `tryExtractFromConsumerRecord parses W3C traceparent header`() {
        ...
        assertEquals("order-created",                     result?.eventType)  // topic → eventType
    }
    ...
    fun `tryExtractFromConsumerRecord falls back to custom headers when no traceparent`() {
        ...
        record.headers().add("X-Issuer",       "custom-svc".toByteArray())
    ...
    @Test
    fun `tryExtractFromConsumerRecord uses topic name as eventType`() {
        val record = ConsumerRecord<String, String>("inventory-updated", 0, 0L, null, "payload")
        record.headers().add("traceparent", "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01".toByteArray())

        val result = tryConsumerRecord(record)
        assertEquals("inventory-updated", result?.eventType)
    }
    ...
    fun `tryExtractFromConsumerRecord with no headers returns metadata with null trace fields`() {
        ...
        assertNull(result.causationId)
        assertEquals("bare-topic", result.eventType)
    }

    @Test
    fun `tryExtractFromConsumerRecord picks last header when duplicates exist`() {
        val record = ConsumerRecord<String, String>("dup-topic", 0, 0L, null, "payload")
        record.headers().add("X-Trace-Id", "first".toByteArray())
        record.headers().add("X-Trace-Id", "last".toByteArray())

        val result = tryConsumerRecord(record)
        assertEquals("last", result?.traceId)
    }
    ...
    @Test
    fun `extractMetadata skips non-matching args before finding match`() {
        val result = extractMetadata(NoTraceFields(), DuckTyped(traceId = "found"))
        assertEquals("found", result.traceId)
    }
}
